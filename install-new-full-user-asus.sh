...
echo "PowerKeyIgnoreInhibited=yes" | sudo tee -a /etc/systemd/logind.conf
echo "SuspendKeyIgnoreInhibited=yes" | sudo tee -a /etc/systemd/logind.conf
...
cat | sudo tee /etc/netplan/02-netcfg.yaml <<EOF
